...
async function setupPlayer() {
  try {
    ...
    playerInstance.on('ready', () => {
      ...
      // localStorage.setItem("jwplayer.mute", 0);
      ...
      document.addEventListener('keydown', (e) => {
        if (e.key === 'ArrowDown') {
          e.preventDefault()
          getChannelList.style.opacity = 1
          ...
          getChannelList.style.opacity = 0
...
document.addEventListener('touchmove', (e) => {
  if (e.target.getAttribute('geturl')) return
  clearTimeout(debounceTimeout); // Limpiar el timeout anterior
  debounceTimeout = setTimeout(() => {
    // Tu código de lógica aquí
    console.log("Touch move completed", e.touches);
    document.querySelector('.test').style.opacity = document.querySelector('.test').style.opacity == 0 ? 1 : 0
  }, debounceDelay);
